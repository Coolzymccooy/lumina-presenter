...
export const BibleStylePicker: React.FC<BibleStylePickerProps> = ({
  ...
  return (
    <div data-testid="bible-style-picker" className="space-y-2">
      {/* Mode row */}
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className={`${labelSz} text-zinc-400 uppercase tracking-widest font-black w-12 shrink-0`}>Style</span>
        {(['classic', 'smart-random', 'preset'] as BibleStyleMode[]).map((m) => (
          <button
            key={m}
            data-testid={`bible-style-mode-${m}`}
            onClick={() => onModeChange(m)}
            className={`px-2.5 ${chipPy} rounded-md ${textSz} font-black uppercase tracking-wide transition-all active:scale-95 ${
              mode === m
        ...
        {mode !== 'classic' && (
          <button
            ...
            className={`ml-auto flex items-center gap-1 px-2.5 ${chipPy} rounded-md ${textSz} font-black uppercase tracking-wide transition-all bg-gradient-to-br from-zinc-800 to-zinc-900 text-amber-300 border border-amber-900/50 hover:text-white hover:border-amber-500/60 hover:from-amber-900/40 hover:to-amber-950/60 active:scale-95 shadow-sm shadow-amber-950/40`}
          >
      ...
      {mode !== 'classic' && (
        <>
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className={`${labelSz} text-zinc-400 uppercase tracking-widest font-black w-12 shrink-0`}>Family</span>
          {/* Auto option */}
          <button
          ...
          {BIBLE_STYLE_FAMILIES.map((f) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-1.5 px-2.5 ${chipPy} rounded-md ${textSz} font-black uppercase tracking-wide transition-all active:scale-95 ${
                  isActive
