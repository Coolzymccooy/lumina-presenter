...
export const ProfileSettings: React.FC<ProfileSettingsProps> = ({ onClose, onSave, onLogout, currentSettings, currentUser }) => {
  ...
  const handleSave = () => {
    ...
    onSave({ churchName, ccli, defaultVersion, theme, remoteAdminEmails, connectionTargetRoles, sessionId: normalizedSessionId, stageProfile, stageFlowLayout, machineMode, slideBrandingEnabled, slideBrandingSeriesLabel, slideBrandingStyle });
    onClose();
  };
                ...
                <>
                  ...
                  <div className="relative rounded-lg overflow-hidden border border-zinc-800 bg-zinc-900" style={{ height: 72 }}>
                    ...
                    {slideBrandingSeriesLabel && (
                      <div className={`absolute left-0 top-0 bottom-0 flex items-center justify-center ${slideBrandingStyle === 'bold' ? 'bg-black/70' : slideBrandingStyle === 'frosted' ? 'bg-black/45' : 'bg-black/38'}`} style={{ width: 22 }}>
                        <span style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)', fontSize: 7, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.82)', whiteSpace: 'nowrap' }}>
                          {slideBrandingSeriesLabel}
                        </span>
                      </div>
                    )}
                    {/* Right strip preview */}
                    {churchName && (
                      <div className={`absolute right-0 top-0 bottom-0 flex items-center justify-center ${slideBrandingStyle === 'bold' ? 'bg-black/70' : slideBrandingStyle === 'frosted' ? 'bg-black/45' : 'bg-black/38'}`} style={{ width: 22 }}>
                        <span style={{ writingMode: 'vertical-rl', fontSize: 7, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.82)', whiteSpace: 'nowrap' }}>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-[9px] text-zinc-600 px-1">Church name (right strip) comes from the Church Name field above.</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {([
                        ...
                        { id: 'frosted', label: 'Frosted', desc: 'Blurred glass effect' },
                      ] as const).map(({ id, label, desc }) => (
                        <button
                          key={id}
                          onClick={() => setSlideBrandingStyle(id)}
